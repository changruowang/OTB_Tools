function [positions, fps] = dsst(params)
    ...
    t_learnRate = params.t_learning_rate;
    s_learnRate = params.s_learning_rate;
...
%             pos = pos + round(([row, col] - center) * currentScaleFactor);
            ...
            if frame == 2,
                kf_params = kf_init(pos(1), pos(2), 0, 0); % 初始化
            else 
                kf_params.z = pos'; %设置当前时刻的观测位置
...
%             pos = pos + params.cell_size * ([vert_delta, horiz_delta] - center)* currentScaleFactor;    
        ...
        if frame == 1
            ...
            sf_num = new_sf_num;
  			...
  			[bg_hist, fg_hist] = updateHistModel(1, patch, window_sz, target_sz,0 ,0 ,0);
            fg_cmp = fg_hist;
        else
            % subsequent frames, update the model
            [occflag, updateMask] = judgeOCC(patch, bg_hist, fg_cmp, target_sz, occflag);
            fg_cmp = updateFgHistAdaption(patch, window_sz, target_sz, fg_cmp, updateMask, occflag);
            [bg_hist, fg_hist] = updateHistModel(2, patch, window_sz, target_sz, bg_hist, fg_hist, 0.04);
            ...
            model_alphaf = (1 - t_learnRate) * model_alphaf + t_learnRate * alphaf;
            model_xf = (1 - t_learnRate) * model_xf + t_learnRate * xf;
            sf_den = (1 - s_learnRate) * sf_den + s_learnRate * new_sf_den;
            sf_num = (1 - s_learnRate) * sf_num + s_learnRate * new_sf_num;
