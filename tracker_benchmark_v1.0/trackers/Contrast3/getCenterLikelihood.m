function result = getCenterLikelihood(object_likelihood, m)
...
%% equivalent MATLAB function
    ...
    result((floor(add_sz(1)/2)+1):(floor(add_sz(1)/2)+size(center_likelihood,1)), (floor(add_sz(2)/2)+1):(floor(add_sz(2)/2)+size(center_likelihood,2))) = center_likelihood;
end
